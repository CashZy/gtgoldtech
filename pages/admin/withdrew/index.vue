<template>
  <div class="h-full flex flex-col">
    <PageHeader title="All Withdrew's List" back-route="/user" />
      ...
      <table v-else class="min-w-full border-collapse border border-gray-300">
        <thead>
          <tr>
            ...
            <th class="py-2 px-4 border-b">Bank Acounts</th>
        ...
        <tbody>
          <tr v-for="(deposit, index) in deposits" :key="index">
            ...
            <td class="py-2 px-4 border-b">{{ deposit.paymentChannel }}</td>
            <td class="py-2 px-4 border-b">{{ deposit.walletId }}</td>
...
const getDeposits = async () => {
  try {
    loading.value = true;
    const res = await Axios({ url: "/api/withdrew" });
...
const acceptDeposit = async (id, userIId) => {
  ...
  if (deposit) {
    ...
    try {
      ...
      loading.value = false;
      Swal.fire("Accepted!", "Withdrew Completed.", "success");
    } catch {
...
const rejectDeposit = (id) => {
  ...
  loading.value = false;
  console.log("Reject withdrew with ID:", id);
  Swal.fire("Rejected!", "Withdrew Rejected.", "error");
};
